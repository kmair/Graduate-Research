        ...
        if debug['outkeys']:
          for k in data['results']['R2'].keys():          
            ...
            if k in data['results']['Q2'].keys():
              ...
              if Q2 <0.5:
                  print("%s: Q2 suggests this is not a predictive model, Q2: %f, R2: %s"%(k, Q2,R2))
              elif diff<0.3:
                  print('%s: The difference of R2-Q2 is  %f. This is an acceptable difference for predictability, Q2: %f, R2: %s'%(k, diff, Q2, R2))
              else:
                  print('%s: The difference of R2-Q2 is %f. The surrogate model is not able to predict the data reliably, Q2: %f, R2: %s'%(k, diff,Q2,R2))
...
def addBasisGroup(type_of_function, input_indices = "", powers = ""):
  ...
  if type_of_function == "CONST":
    ...
    if powers == "":
      powers = "-1111"
    group_list.append("%i %s %s %s"%(group_index,type_of_function, input_indices,powers))
...
def addBasisConstraint(group_id, output_id, constraint_type, intParam):
  ...
  basis_constraint_list.append("%i %i %s %i"%(group_id, output_id, constraint_type, intParam ))
...
def addCustomConstraints(constraint_list, **kwargs): # function using x and z that is less than 0
  """
