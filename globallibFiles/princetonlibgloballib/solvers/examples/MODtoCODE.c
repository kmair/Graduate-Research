/*
------------------------------------------------------------------
 .mod to C code program converter
 
 modtocode is an AMPL solver
 The purpose is to read a model file (.mod) and produce C code version.
 The code is expected to be used along with the script mydfo.
 {\tt modtocode} consists of a main program, which provides input, output 
 and execution for the {\tt nlc} function by D. Gay \cite{g97} that reads 
 the .mod file.
 
 The code  is called 'truth.exe' and is delivered in the same directory
 where AMPL resides. 
...
//-------------------------------- 
system("cat black_box.c black_main.c > amplmodel_code.c");
