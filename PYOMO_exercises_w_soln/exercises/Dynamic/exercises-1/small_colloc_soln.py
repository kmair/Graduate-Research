...
m.colloc = RangeSet(0,numcollocation-1)
m.fe = RangeSet(0,numfinelems-1)
m.h = Param(initialize=1.0/numfinelems)

m.z = Var(m.fe,m.colloc)
m.dzdt = Var(m.fe,m.colloc)
m.time = Var(m.fe,m.colloc)
...
m.zdot = Constraint(m.fe,m.colloc,rule=_zdot)
...
m.colloc_eq = Constraint(m.fe,m.colloc,rule=_colloc_eq)

def _colloc_eq_t(m,i,j):
    if j == 0:
        return Constraint.Skip
    return m.h == sum(m.time[i,k]*adot[k][j] for k in range(0,numcollocation))
m.colloc_eq_t = Constraint(m.fe,m.colloc,rule=_colloc_eq_t)
...
m.cont_z = Constraint(m.fe,rule=_cont_z)

def _cont_t(m,i):
    if i == 0:
        return Constraint.Skip
    return m.time[i,0] == m.time[i-1,numcollocation-1]
m.cont_t = Constraint(m.fe,rule=_cont_t)
...
def _init_con_t(m):
    return m.time[0,0] == 0
m.init_con_t = Constraint(rule=_init_con_t)
...
for i in range(0,numfinelems):
    for j in range(0,numcollocation):
        if i != 0 and j == 0:
            continue
        findiff_t.append(value(m.time[i,j]))
        findiff_z.append(value(m.z[i,j]))
