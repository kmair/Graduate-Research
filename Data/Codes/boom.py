...
class BCD():
    def __init__(self, xt, n_attr, data, Numpoints, xt_1 = None, fnCalls = 0, Present_iter = 0, n_blocks = None, var_order = None, 
                 ...
                 SP_dict = None, max_calls = 2500, has_exe = True):    
    ...
    def xl_writer(self, action, to_write = False):
        ...
        if to_write:                
            ...
            data_path = os.path.join(dirpath, LIBRARY)
            ...
            print('Not writing iteration result')

    def xl_data_writer(self, action, X, Z, to_write = False):
        '''
        ...
        if to_write:                
            ...
            path = os.getcwd()                      # C files location: source_princetonlibgloballib
            xl_file = self.data.compileFile + '.xls' 
            sh_name = 'all_attr' + str(self.n_attr) + 'pts' + str(self.Numpoints) #+ '_beta' + str(self.Beta) # We're keeping beta same everywhere
...
def newPoint(max_calls, n_attr, data = None, Numpoints = 20, fnCalls= 0, max_epochs = 20, SP_dict = None, numberSP = 5):
    ...
    try:
        ...
        del SP_dict[z_min]
        data = Sampling.Trust_Region_method(compileFile, numofVar, Upper, Lower, numberSP, method = 'SOBOL', StartPt = StartPt) # Scaling_factor = 1,
...
'''
Ex use:
python boom.py denschnb 26 1 (10)
