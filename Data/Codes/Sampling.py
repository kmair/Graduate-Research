...
def Variable_Data(compileFile):
    ...
    # file_path = os.path.join(folder_path, 'source_convexmodels')
...
def fn_call(X, compileFile, LIBRARY, compileFlag=0):                         # X is a (1,m) vector
    
    if compileFile == 'ASPEN':
        import aspen_prob
        # here LIBRARY would be args = (hyCase, hysolver)
        y_data = aspen_prob.hy_Object(*LIBRARY, X)
        print('function output of x0 is: ',y_data)
    ...
    file_path = os.path.join(dirpath, LIBRARY)
    os.chdir(file_path)
